...
则 \(\mathbf{v}\) 为特征向量，\(\lambda\) 为特征值。矩阵变换在该方向上只做缩放、不改变方向。特征值的大小决定了系统经过多层变换后信号是放大（\(\lambda > 1\)）还是衰减（\(\lambda \lt 1\)），直接关联到梯度爆炸与消失。
...
$$
\mathcal{L}_{NLL} = - \sum_{t=1}^T \log P_\theta(x_t \mid x_{\lt t})
$$
